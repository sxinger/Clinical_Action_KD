...
/*
/*in: ED_SI_6HR_screen@deid, encounter_mapping, patient_mapping, patient_dimension, heron_etl_1.kuh_med_orders, clarity.mar_admin_info, clarity.zc_mar_rslt,PRESSOR_CD
/*
...
join &&i2b2_db_schemadata.encounter_mapping ide on e.encounter_num = ide.encounter_num and
...
join &&i2b2_db_schemadata.patient_mapping pm on epic.patient_ide = pm.patient_ide and
...
select n.encounter_num
      ...
      ,p.date_shift
from ide_pn_cw n
join &&i2b2_db_schemadata.patient_dimension p on n.patient_num = p.patient_num
)
     ,mar_vaso_order as (
...
join clarity.zc_mar_rslt cmr on mar.mar_action_c = cmr.result_c
where cmr.name not in ('Stopped','Dropped','Canceled Entry','Missed','Held','Med Not Available','See ED Trauma Flowsheet')
