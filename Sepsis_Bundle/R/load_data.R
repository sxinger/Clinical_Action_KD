...
for(i in seq_along(chunk_id$CONCEPT_PREFIX)){
  ...
  chk_i<-dbGetQuery(conn,
    ...
    inner_join(enroll %>% dplyr::select(PATIENT_NUM,ENCOUNTER_NUM,TRT3HR_COMPLT_FAST_IND),
  ...
  if(chunk_id$CONCEPT_PREFIX[i] %in% c("KUH|MEDICATION_ID")){
    data_i<-chk_i %>%
      ...
      dplyr::select(PATIENT_NUM,ENCOUNTER_NUM,VARIABLE,NVAL_NUM,TVAL_CHAR,START_SINCE_TRIAGE,TRT3HR_COMPLT_FAST_IND,CONCEPT_CD) %>% 
      unique %>%
      bind_rows(chk_i %>%
                  filter(is.na(NVAL_NUM) & !is.na(UNITS_CD)) %>%
                  mutate(CONCEPT_CD2=CONCEPT_CD) %>%
  ...
  }else if(chunk_id$CONCEPT_PREFIX[i] %in% c("KUH|DI","KUH|DX_ID","ICD","KUH|PROC_ID")){
    data_i<-chk_i %>%
      mutate(CONCEPT_CD2=CONCEPT_CD) %>%
      unite("VARIABLE",c("CONCEPT_CD2","MODIFIER_CD"),sep="@") %>%
      dplyr::mutate(NVAL_NUM=1) %>%
    ...
    data_i<-chk_i %>%
      ...
      bind_rows(chk_i %>%
                  filter(!is.na(NVAL_NUM)) %>%
                  group_by(PATIENT_NUM,ENCOUNTER_NUM,VARIABLE,TVAL_CHAR,START_SINCE_TRIAGE,TRT3HR_COMPLT_FAST_IND,CONCEPT_CD) %>%
  ...
  feat_i<-data_i %>%
    ...
    dplyr::summarize(enc_wi = length(unique(ENCOUNTER_NUM)),
                     ...
                     pos_wi = length(unique(ENCOUNTER_NUM*TRT3HR_COMPLT_FAST_IND))-1,
                     pos_wo = P-(length(unique(ENCOUNTER_NUM*TRT3HR_COMPLT_FAST_IND))-1),
...
flo_meas_cd<-readRDS("../Suspected_Infection/data/additional_fs_cd.rda")
...
data_at_enc %<>%
  dplyr::select(-TRT3HR_COMPLT_FAST_IND) %>%
  bind_rows(pat_at_enc2)

#------------augment feature dictionary with patient-level info
feat_at_enc2 %<>%
  bind_rows(pat_at_enc2 %>%
              left_join(enroll %>% dplyr::select(PATIENT_NUM,ENCOUNTER_NUM,TRT3HR_COMPLT_FAST_IND),
                        by=c("PATIENT_NUM","ENCOUNTER_NUM")) %>%
...
for(i in seq_along(chunk_id$CONCEPT_PREFIX)){
  ...
  data_i<-chk_i %>%
    dplyr::mutate(VARIABLE=CONCEPT_CD) %>%
    dplyr::select(PATIENT_NUM,ENCOUNTER_NUM,VARIABLE,DAY_BEF_TRIAGE,TRT3HR_COMPLT_FAST_IND) %>% 
    unique
