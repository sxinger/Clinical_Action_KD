...
union all
select 37 as idx,'NonHypotensive-Sepsis before Therapautic Components' CNT_TYPE,
       count(distinct patient_num) PAT_CNT,
       count(distinct encounter_num) ENC_CNT
from ED_SI_SEPSIS_STAGE
...
union all
select 38 as idx,'Repeat Lactate' CNT_TYPE,
       count(distinct patient_num) PAT_CNT,
       count(distinct encounter_num) ENC_CNT
from ED_SI_SEPSIS_STAGE
where LACTATE2_since_triage is not null
union all
select 39 as idx,'Remain Hypotensive' CNT_TYPE,
       count(distinct patient_num) PAT_CNT,
       count(distinct encounter_num) ENC_CNT
from ED_SI_SEPSIS_STAGE
where hypot2_since_trigger is not null
union all
select 40 as idx,'Vasopressor initiated' CNT_TYPE,
       count(distinct patient_num) PAT_CNT,
       count(distinct encounter_num) ENC_CNT
from ED_SI_SEPSIS_STAGE
where pressor_since_triage is not null
union all
select 41 as idx,'Cardiac events' CNT_TYPE,
       count(distinct patient_num) PAT_CNT,
       count(distinct encounter_num) ENC_CNT
from ED_SI_SEPSIS_STAGE
